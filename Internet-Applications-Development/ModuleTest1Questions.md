...
В этом есть и плюсы и минусы. К плюсам можно отнести перфоманс, к минусам то, что если что-то сломается, то в CGI упадет один процесс и (наверное) этого никто не заметит, в FastCGI упадет все :)
...
JavaScript является объектно-ориентированным языком. Js имеет автоматическое приведение типов, автоматическую сборку мусора, анонимные функции, функции как объекты первого класса (т.е. мб сохранены в переменную, переданны в функцию как аргумент, созданы во время выполнения программы и т.п.)
...
1. Клиент запрашивает страницу у сервера, используя обычный http
2. Jткрывается соединение с сервером
