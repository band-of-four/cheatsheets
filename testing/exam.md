...
Входные значения - данные или управляющие последовательнсти.
...
## Уровень 0 – тестирование == отладка, работка с отладчиком
  ...
  – Не отличает некорректное поведение и ошибки в программе (программа может вести себя некорректно, однако в дебаггере не будет видно ошибки, так как не производится
...
## Уровень 2 - Демонстрация ошибок
  ...
  Конфликт разработчиков и тестировщиков. Поиск дефектов ради поиска дефектов, доказательства некорректности работы модуля. Поощрения за найденные дефекты,
  конфронтация разработчиков и тестировщиков вместо кооперации во имя повышения качества итогового продукта.
...
__Тестирование методом черного ящика__ - При тестировании чёрного ящика тестировщик имеет доступ к программе только через те же интерфейсы, что и заказчик или пользователь, либо через внешние интерфейсы, позволяющие другому компьютеру либо другому процессу подключиться к системе для тестирования.
...
"равноправный анализ". Внутри выбираются метрики по которым оцениваются артефакты

Есть четко определенные шаги: вход, планирование, обзор, (подготовка, обсуждение, переработка, выработка рекомендаций), выход

1. Входные и выходные критерии должны быть определены. Входными критериями могут быть: подготовка чеклистов, проверка документов на ошибки.  Выходные критерии: документы согласованы

2. Планирование инспекции: планирование check-rate, расписание, учасники, собираемые метрики 
...
Подробнее о ролях: идея в том, что каждый человек может одновременно быть сконцентрированным максимум на двух проблемах. Поэтому каждому инспектору выдают какое-то четкое задание, на котором он будет сконцентрирован.
...
5. Обсуждение результатов: основная цель - протоколирование результатов. Ведщий должен быть дипломатичен и краток и не превращать обсуждение в бесконечную дискуссию. После обсуждения выносится решение "принят"/ "не принят"
...
- Определены критерии входа и выхода для приемки программного продукта

- Предварительная подготовка перед встречей

- Инспекционный отчет включает список найденных проблем и вопросов

- Формальный процесс отслеживания oшибок. Необязательно - процесс улучшения компонентов
...
1. Представляем логику нашего модуля через диаграмму состояний - граф, в котором узлы - состояния, а ребра - события, которые триггерят переход.
2. Выписываем все состояния и события и делаем декартово произведение. Получаем здоровенную таблицу.
3. Анализируем каждую строку этой таблицы. Валидно ли это событие при этом состоянии? Если нет ладно, если да - в какое состояние мы должны перейти
и что мы ожидаем что должно произойти?
4. Ответив на эти вопросы для строки можно писать тест.
...
Если входные параметры описываются структурами данных, которые неудобно представлять в формате `.csv` файла или объявлять
